  ...
  methods: {
    async submitForm() {
      ...
      if (!this.errorMessage) {
        console.log('Nuevos Ajustes de membresía: ', formData);
        ...
        if (response.status === 200) {
          this.$router.push({
            ...
            query: {
              ...
              ruc: this.ruc,
              direction: this.direction,
              tipoMembresia: this.formData.tipoMembresia, // Pasar el valor a través de la URL
              tipoTarjeta: this.tipoTarjeta
            }
          });
          console.log('Datos que se envían a la API:', updatedCompanyData);
        } else {
